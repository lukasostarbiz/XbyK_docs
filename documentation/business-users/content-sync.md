---
source: https://docs.kentico.com/documentation/business-users/content-sync
...
[![Instances connected using content sync](/docsassets/documentation/content-sync/content_sync_topology.png)](/docsassets/documentation/content-sync/content_sync_topology.png)
**Prerequisites**
...
Pages  
  ...
  * [Forms](/documentation/business-users/digital-marketing/forms) added to [Page Builder content](/documentation/business-users/website-content/widgets-and-page-builder) via the _Form_ widget 
    ...
    * Forms that are [rendered](/documentation/developers-and-admins/development/builders/page-builder/render-widgets-in-code) directly in the page code (not added in the Page Builder interface) are not synchronized together with pages. Such forms can be [synchronized](#forms) separately from the _Forms_ application.
...
### Content items
  ...
  3. Use the **Sync** (  
[![Sync content items](/docsassets/documentation/content-sync/sync_content_items.png)](/docsassets/documentation/content-sync/sync_content_items.png)
  4. Choose [when you want the content to be restored](#choose-the-restoration-time-for-synchronized-content) on the target instance.
...
[![Sync pages](/docsassets/documentation/content-sync/sync_pages.png)](/docsassets/documentation/content-sync/sync_pages.png)
  5. Choose [when you want the content to be restored](#choose-the-restoration-time-for-synchronized-content) on the target instance.
...
Forms can be synchronized together with pages that display them in their [Page Builder content](/documentation/business-users/website-content/widgets-and-page-builder). In this scenario, use the synchronization instructions described for [Pages](#pages), with the **Include forms** option enabled. This also applies when synchronizing [Content items](#content-items), where forms can be synchronized via linked pages.
[![Include forms when synchronizing pages and content items](/docsassets/documentation/content-sync/content_sync_include_forms.png)](/docsassets/documentation/content-sync/content_sync_include_forms.png)
Forms are not synchronized together with pages if they are [rendered](/documentation/developers-and-admins/development/builders/page-builder/render-widgets-in-code) directly in the page code (not added in the Page Builder UI). Such forms can be [synchronized separately](#sync-forms-individually) in the _Forms_ application.
...
You can synchronize individual forms separately, without pages or content items:
  ...
  2. Use the **Sync** (  
[![Sync a form](/docsassets/documentation/content-sync/sync_form.png)](/docsassets/documentation/content-sync/sync_form.png)
  3. Choose [when you want the form to be restored](#choose-the-restoration-time-for-synchronized-content) on the target instance.
  4. Select **Sync**. 
...
Whenever you synchronize content, you can choose when the data will be restored on the target instance:
  ...
  * **Postpone restoration on the target** – select a date and time in the future. The content will be transferred to the target instance and restored at the scheduled time. You can [track scheduled synchronizations](#track-incoming-synchronizations) on the target instance.


[![Choosing when to restore synchronized content](/docsassets/documentation/content-sync/content_sync_restore_time.png)](/docsassets/documentation/content-sync/content_sync_restore_time.png)
...
[![View planned or performed synchronizations](/docsassets/documentation/content-sync/content_sync_scheduled.png)](/docsassets/documentation/content-sync/content_sync_scheduled.png)
