# Content items
  * [ Copy page link ](documentation/business-users/content-hub/content-items#) | [Get HelpService ID](documentation/business-users/content-hub/content-items#)
Core MVC 5


[✖](documentation/business-users/content-hub/content-items# "Close page link panel") [Copy to clipboard](documentation/business-users/content-hub/content-items#)
...
[![Filtering items in the Content hub application](docsassets/documentation/content-items/CH_filter.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_filter.png)
## Create content items
  1. Select **New content item**.  
[![Creating a new content item](docsassets/documentation/content-items/CH_create_item.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_create_item.png)
...
[![Choosing the content item name, location and type](docsassets/documentation/content-items/ContentItemCreate.png)](https://docs.kentico.com/docsassets/documentation/content-items/ContentItemCreate.png)
  2. Select **Continue**.
...
You can directly **Publish** new items to make them available for your audience (either immediately or at a scheduled time). See [Publish content items](documentation/business-users/content-hub/content-items#publish-content-items) to learn more.
If the item has fields [linking](documentation/business-users/content-hub/content-items#link-content-items) to other reusable content items, the cascade publishing dialog opens when publishing. You can view all linked content items you have access to, that are currently not published, and select them for publishing together with the main item.
...
You can clone all [language variants](documentation/business-users/content-hub/content-items#translate-content-items) of an item or just a selected language variant.
To clone a content item:
  ...
  3. Select the **Clone** ([![Clone action](docsassets/documentation/content-items/CH_clone.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_clone.png)
...
You can [find](documentation/business-users/content-hub/content-items#find-content-items) the new item under the name you selected. Language variants other than the one selected during the cloning are automatically named using the pattern _(Clone) {original name}_. If you cloned an item located inside a [content folder](documentation/business-users/content-hub/content-hub-folders#content-folders), you can find it under **All content items**.
The new item is created in the _Draft_ step, where you can [edit](documentation/business-users/content-hub/content-items#edit-content-items) it. The item needs to be [published](documentation/business-users/content-hub/content-items#publish-content-items) before it can be delivered to your audience.
## Edit content items
...
[![Edit a Published or Scheduled content item](docsassets/documentation/content-items/CH_edit_item.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_edit_item.png)
     * When a content item is _Unpublished_ , select **Create new version** to move the item to the _Draft_ step.  
[![Create new version of a content item](docsassets/documentation/content-items/CH_create_new_version.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_create_new_version.png)
...
To move content items:
  ...
  2. Select the content items you want to move. 
     * Use the search bar, filter or [folders](documentation/business-users/content-hub/content-hub-folders) to [find content items](documentation/business-users/content-hub/content-items#find-content-items).
  3. Select the **Move** (  
[![Move content items](docsassets/documentation/content-items/move_content_items.png)](https://docs.kentico.com/docsassets/documentation/content-items/move_content_items.png)
...
Content items may support linking other items to create nested hierarchies. For example, a content item representing a medical practitioners office can link to other content items representing employed doctors, patient testimonials, and so on. These relationships are determined by the content model used by your project.
  1. [Edit](documentation/business-users/content-hub/content-items#edit-content-items) a content item that supports referencing other content items.
...
[![Link content items to a content item](docsassets/documentation/content-items/CH_page_link.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_page_link.png)
  3. **Save** the changes.


The content items are now linked to the edited item. Each linked item also respects its status – it may not be immediately active upon being linked. Linked items can be published together with the main item using [Cascade publishing](documentation/business-users/content-hub/content-items#cascade-publishing). To learn how the lifecycle influences linked items, see [Content items lifecycle](documentation/business-users/content-hub/content-items#content-items-lifecycle).
...
Depending on the configuration of your project, content items may support [taxonomies](documentation/developers-and-admins/configuration/taxonomies) and tags.
  1. [Edit](documentation/business-users/content-hub/content-items#edit-content-items) a content item that has a field with the tag selector.
  2. **Select tags** that describe the content item.  
[![Tag selector](docsassets/documentation/content-items/tag_selector.png)](https://docs.kentico.com/docsassets/documentation/content-items/tag_selector.png)
  3. **Save** the changes.
...
[![Properties tab](docsassets/documentation/content-items/properties_tab.png)](https://docs.kentico.com/docsassets/documentation/content-items/properties_tab.png)
## Track usage of content items
Even with thousands of content items in your project, the Content reuse locator feature helps you monitor where each item is [linked](documentation/business-users/content-hub/content-items#link-content-items) and which channels display it. Understanding a content item’s usage is valuable when assessing the potential impact of edits or deletions. For instance, you can verify whether a content item you plan to delete appears in published content and whether removing it might disrupt that content.
...
[![Content hub usage tab](docsassets/documentation/content-items/CH_Usage.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_Usage.png)
...
To publish a content item:
  ...
  4. Choose when you want to publish the item: 
     * **Publish now**
     * **Schedule publish** – select a date and time in the future. The system will automatically publish the item at the given time (see [Scheduled publishing](documentation/business-users/content-hub/content-items#scheduled-publishing)).
  5. Select **Confirm**. 
     * If the item has fields [linking](documentation/business-users/content-hub/content-items#link-content-items) to other reusable content items or pages, the [cascade publishing](documentation/business-users/content-hub/content-items#cascade-publishing) dialog opens.
...
[![Scheduling the time when an item will be published](docsassets/documentation/content-items/CH_schedule_publish.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_schedule_publish.png)
...
[![Mass publish confirmation dialog](docsassets/documentation/content-items/CH_mass_publish_confirm.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_mass_publish_confirm.png)
To confirm the mass publish, select **Publish** or **Continue** (appears if [cascade publishing](documentation/business-users/content-hub/content-items#cascade-publishing) of linked items is possible for any of the published items).
### Cascade publishing
When publishing (or scheduling the publish for) content items with fields [linking](documentation/business-users/content-hub/content-items#link-content-items) to other reusable content items or pages, the cascade publishing dialog opens. Here you can view all items that are associated with the item you’re about to publish, but are currently not published. If you are publishing multiple items using [mass publishing](documentation/business-users/content-hub/content-items#mass-publishing), the dialog displays linked items for all published items (in this case, the main published items are not listed).
You can select [linked items](documentation/business-users/content-hub/content-items#link-content-items) to be published together with the main items. Linked items that currently do not fulfill the requirements for publishing are displayed, but cannot be selected. Leaving linked items unpublished may lead to missing data or other issues in the content of the published items.
[![Cascade publishing](docsassets/documentation/content-items/Cascade_Publishing.png)](https://docs.kentico.com/docsassets/documentation/content-items/Cascade_Publishing.png)
**Cascade scheduled publishing**
If you are [scheduling](documentation/business-users/content-hub/content-items#scheduled-publishing) an item to be published, any linked items that you select in the cascade publishing dialog are scheduled to be published at the same time as the main item. This overwrites the publish time for any linked items that are already scheduled to be published.
Linked items that are scheduled to be [unpublished](documentation/business-users/content-hub/content-items#unpublish-content-items) at a time before the new scheduled publish date **cannot be selected** in the cascade publishing dialog.
...
At any time before the new version is published, you can revert the item back to the last _Published_ status. All changes made in any workflow step will be lost.
  ...
  4. Select **Revert to published** to revert the changes. [![Reverting a content item to its published status](docsassets/documentation/content-items/CH_revert_to_published.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_revert_to_published.png)
...
[![Content language selector](docsassets/documentation/content-items/header_bar_language_switcher.png)](https://docs.kentico.com/docsassets/documentation/content-items/header_bar_language_switcher.png)
...
Untranslated content items are visually marked by the _Not translated_ (
  5. Enter the translated name of the content item. You can now choose one of the following options:
     ...
     * **Translate using AIRA** see [Translate using AIRA](documentation/business-users/content-hub/content-items#translate-using-aira) for more information.  
[![Translate content item](docsassets/documentation/content-items/translate_content_item.png)](https://docs.kentico.com/docsassets/documentation/content-items/translate_content_item.png)
  6. Select **Continue**.
  7. Enter the translated values of the content item’s fields.
  8. **Save** the translated content item.


You have now successfully translated the content item. Note that the particular language variant is now only a _Draft_ and so won’t be accessible on the live site until you [publish](documentation/business-users/content-hub/content-items#publish-content-items) it. Until you do so, the language variant in the fallback language will be displayed.
...
[![Content item security settings](docsassets/documentation/content-items/ContentItemSecuritySettings.jpg)](https://docs.kentico.com/docsassets/documentation/content-items/ContentItemSecuritySettings.jpg)
  5. Select **Save**.
...
To unpublish a content item:
  ...
  2. Select a content item you want to unpublish. 
     * You can only unpublish content items that are already [published](documentation/business-users/content-hub/content-items#publish-content-items) or [scheduled to be published](documentation/business-users/content-hub/content-items#scheduled-publishing) in the future.
     * Use the search bar, filter or [folders](documentation/business-users/content-hub/content-hub-folders) to [find content items](documentation/business-users/content-hub/content-items#find-content-items).
  3. Expand the **Edit content item** menu and select **Unpublish options**.  
[![Unpublish a content item](docsassets/documentation/content-items/CH_unpublish.png)](https://docs.kentico.com/docsassets/documentation/content-items/CH_unpublish.png)
...
To [edit](documentation/business-users/content-hub/content-items#edit-content-items) or re-publish an unpublished content item, select **Create new version** for the content item. This creates a new version of the item in the _Draft_ step.
...
For items that have a scheduled unpublish time while they are still [scheduled for publishing](documentation/business-users/content-hub/content-items#scheduled-publishing), canceling the scheduled publish (or editing the item) also **automatically cancels the scheduled unpublish**.
...
In the current version of Xperience by Kentico, deleted items can be [restored](documentation/business-users/recycle_bin). If you’re using a version older than 30.12.0, deleting a content item is non-reversible and the item cannot be restored.
  ...
  3. Select **Move to recycle bin** (
     * If the deleted content item is used in other content, you may want to [check the usage](documentation/business-users/content-hub/content-items#track-usage-of-content-items) of the item and edit all content where it is used.
     * **Tip** : You can quickly see whether a content item is used by checking the **In use** column of the content item listing.


[![Delete content items](docsassets/documentation/content-items/ContentHubDelete.png)](https://docs.kentico.com/docsassets/documentation/content-items/ContentHubDelete.png)
...
[![Content availability for individual statuses](docsassets/documentation/content-items/WorkflowStatesSummary.png)](https://docs.kentico.com/docsassets/documentation/content-items/WorkflowStatesSummary.png)
The same general behavior also applies to [linked content items](documentation/business-users/content-hub/content-items#link-content-items). A content item in the **Draft** status linked to a content item in the **Published** status will only show up in content previews (not in live content) until it transitions to **Published**.
The following flowchart demonstrates available status transitions:
[![Status transitions flowchart](docsassets/documentation/content-items/final_workflow.png)](https://docs.kentico.com/docsassets/documentation/content-items/final_workflow.png)
### Advanced scenarios
This section looks at a couple of example scenarios and explains how they can be influenced by statuses.
  * [Mass translating](documentation/business-users/content-hub/content-items#mass-translating)
  * [Content item with linked items](documentation/business-users/content-hub/content-items#content-item-with-linked-items)
  * [Creating new drafts of published (linked) content items](documentation/business-users/content-hub/content-items#creating-new-drafts-of-published-linked-content-items)
  * [Content item assets](documentation/business-users/content-hub/content-items#content-item-assets)
  * [Publishing a content item containing non-published content items](documentation/business-users/content-hub/content-items#publishing-a-content-item-containing-non-published-content-items)
  * [Publishing a content item containing items with a new draft version](documentation/business-users/content-hub/content-items#publishing-a-content-item-containing-items-with-a-new-draft-version)


#### Content item with linked items
**Scenario:** I have a content item with multiple [linked content items](documentation/business-users/content-hub/content-items#link-content-items). How does the status of the linked items impact my main item?
...
[![Linked content items lifecycle behavior](docsassets/documentation/content-items/LinkedItemWorkflow.png)](https://docs.kentico.com/docsassets/documentation/content-items/LinkedItemWorkflow.png)
