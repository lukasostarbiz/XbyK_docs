...
Once you have installed the project templates package, you can install projects using the .NET command-line interface. You need to know [the template short name for the corresponding project type](#available-project-templates).
  ...
  3. Run the following command:
     * Replace _< short_name>_ with the template short name of the project type you want to install. See [Available project templates](#available-project-templates).
...
The `kentico-xperience-dbmanager` command is installed when [creating a project](#Installation-CreateProject). Run the `dotnet tool restore` command from the project’s root directory if the command is unavailable.
CMD
