# Continuous Integration
  * [ Copy page link ](documentation/developers-and-admins/ci-cd/continuous-integration#) | [Get HelpService ID](documentation/developers-and-admins/ci-cd/continuous-integration#)
Core MVC 5


[✖](documentation/developers-and-admins/ci-cd/continuous-integration# "Close page link panel") [Copy to clipboard](documentation/developers-and-admins/ci-cd/continuous-integration#)
...
To start developing projects using CI:
  1. Set up your development environment and enable CI.
     * [Prepare the development environment](documentation/developers-and-admins/ci-cd/continuous-integration#prepare-the-development-environment)
  ...
  2. Establish a process for transferring objects back into the database from the repository XML files.
     * [Restore repository files to the database](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database)
...
Start by preparing an initial instance:
  1. If using Git for Windows as your source control solution, consider enabling long paths support: see [Long paths support in Git for Windows](documentation/developers-and-admins/ci-cd/continuous-integration#long-paths-support-in-git-for-windows).
  ...
  3. Enable CI on the given instance (see [Enable Continuous Integration](documentation/developers-and-admins/ci-cd/continuous-integration#enable-continuous-integration) for details).
...
– OR –
  * Use the original database backup and then get the latest data:
    1. Restore the current object status from the CI repository into the database. See [Restore repository files to the database](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database).
...
To start using Continuous Integration on an Xperience instance:
  ...
  4. **Save** the changes.
  5. [Store all object data to the Continuous Integration repository](documentation/developers-and-admins/ci-cd/continuous-integration#store-object-data-to-the-repository).
...
If you undo a delete or modify operation and revert an XML file to its previous state, you then need to [restore the objects into the database](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database) to reverse the object changes in Xperience.
...
Whenever you load (pull) a different version of any files in the CI repository folder from your source control, you need to:
  1. Merge any files that conflict with your instance’s local versions. See also: [Resolving object conflicts](documentation/developers-and-admins/ci-cd/continuous-integration#resolve-conflicts)
  2. [Restore the objects into your database](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database).
...
**Note** :
  ...
  * If you change an object’s GUID during conflict resolution and then [restore the object](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database) to your database, you may need to manually fix broken references on your local development instance. For example, object and page fields may reference files via the GUID of the selected file.
...
If you delete an object and then create a new object of the same type with an identical [identifier](documentation/developers-and-admins/ci-cd/continuous-integration#object-identifiers), you may encounter errors when restoring the CI data. For example, such problems can occur if you delete a _/Home_ page, and then create a new /_Home_ page with a different content type.
...
If you need to apply an [update](documentation/developers-and-admins/installation/update-xperience-by-kentico-projects) to an instance that has CI enabled, use the following procedure:
  1. [Restore objects from the Continuous Integration repository to your database](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database).
You can skip the restore if you are sure that your database is synchronized with the current status of your repository folder.
  2. Disable CI (e.g., using the [settings](documentation/developers-and-admins/ci-cd/continuous-integration#enable-continuous-integration) in the Xperience administration).
  3. Apply the update.
  4. Re-enable CI.
  ...
  6. [Store object data to the repository](documentation/developers-and-admins/ci-cd/continuous-integration#store-object-data-to-the-repository) to recreate the serialized content.
...
For major updates, it may be more efficient to only update one instance and then set up your environment again according to the process described in [Prepare the development environment](documentation/developers-and-admins/ci-cd/continuous-integration#prepare-the-development-environment).
...
When serializing object data to the file system or [restoring data](documentation/developers-and-admins/ci-cd/continuous-integration#restore-repository-files-to-the-database) back to the database, the system now uses the specified folder as the root of the CI repository. You can add the folder into your source control system.
