---
source: https://docs.kentico.com/documentation/developers-and-admins/ci-cd/ci-cd-database-migration-scripts
...
When you need to create or modify objects not supported by CI/CD (for example table indexes, functions, or stored procedures) and you want to synchronize or deploy these changes, you need to:
  1. Write a migration script that applies the required change to the database. Refer to [Example - Database change migration script](#example---database-change-migration-script) for an example of the process.
...
The rollback process is demonstrated here by extending the [example above](#example---database-change-migration-script). To revert the changes made by the **AddOfficeNameIndex.sql** migration, you need to:
