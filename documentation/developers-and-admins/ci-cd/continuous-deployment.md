# Continuous Deployment
  * [ Copy page link ](documentation/developers-and-admins/ci-cd/continuous-deployment#) | [Get HelpService ID](documentation/developers-and-admins/ci-cd/continuous-deployment#)
Core MVC 5


[✖](documentation/developers-and-admins/ci-cd/continuous-deployment# "Close page link panel") [Copy to clipboard](documentation/developers-and-admins/ci-cd/continuous-deployment#)
...
Deployment with CD uses the following general process:
  ...
  2. Configure the development instance to correctly [synchronize macro signatures](documentation/developers-and-admins/ci-cd/continuous-deployment#synchronize-macro-signatures) with the target deployment instances.
  3. [Generate a CD configuration file](documentation/developers-and-admins/ci-cd/continuous-deployment#generate-cd-configuration-files) and adjust it according to your requirements.
  4. [Store database objects to a CD repository](documentation/developers-and-admins/ci-cd/continuous-deployment#store-objects-to-a-cd-repository) using the appropriate configuration file for your deployment.
  5. Run your deployment process or pipeline for the project. [Restore the CD repository](documentation/developers-and-admins/ci-cd/continuous-deployment#restore-the-cd-repository-to-the-database) to the target environment as part of the overall process.
...
Continue by using the configuration file when you [store objects to a CD repository](documentation/developers-and-admins/ci-cd/continuous-deployment#store-objects-to-a-cd-repository).
**CD restore mode**
...
The `--kxp-cd-store` command has the following parameters: 
       ...
       * `--config-path` – file system path of the [CD configuration file](documentation/developers-and-admins/ci-cd/continuous-deployment#generate-cd-configuration-files), including the file name. Required if storing objects to a new repository folder. When storing to an existing repository, the parameter is optional – if not specified, the system searches for a **repository.config** file in the root of the repository folder. Do not explicitly specify the `--config-path` parameter if you want to use the configuration file in the root of the repository folder.
Add the `--` syntax separator before any command parameters that you wish to pass directly to the Xperience application and avoid conflicts with .NET CLI parameters.
...
The CD repository can then be used to [restore the data](documentation/developers-and-admins/ci-cd/continuous-deployment#restore-the-cd-repository-to-the-database) to a target environment’s database.
...
For [deployment to private cloud](documentation/developers-and-admins/deployment/deploy-to-private-cloud), the simplest solution is to have matching `CMSHashStringSalt` values for your development project and all target deployment instances. If this is not possible, we recommend the following approach:
  ...
  4. Extend your CD process to [re-sign](documentation/developers-and-admins/configuration/macro-expressions/macro-signatures) macros **before** you [store objects](documentation/developers-and-admins/ci-cd/continuous-deployment#store-objects-to-a-cd-repository) to a repository.
...
The restore action deserializes objects from the CD repository and updates the target database according to the [restore mode set in the repository’s configuration file](documentation/developers-and-admins/ci-cd/continuous-deployment#generate-cd-configuration-files).
...
Depending on the hosting environment to which you are deploying, you can run the CD restore from the following locations:
  * **From your development environment** – when you have your project ready for deployment, run the restore command from the given project and [specify the target production database](documentation/developers-and-admins/ci-cd/continuous-deployment#set-the-target-database).
