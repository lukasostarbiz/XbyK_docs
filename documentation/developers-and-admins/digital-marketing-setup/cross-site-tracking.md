...
## Set up cross-site tracking
  1. [Enable the cross-site tracking feature for your Xperience application](#enable-the-cross-site-tracking-feature)
  2. [Register external sites that you wish to track in Xperience](#manage-tracked-sites)
  3. [Add a tracking snippet to the tracked sites](#add-the-tracking-snippet)
...
Cross-site tracking works through client scripts that send logging requests from external sites. You need to add a code snippet containing the required scripts to every tracked website:
  1. Open the **Cross-site tracking** application in the Xperience administration.
  2. Select the [tracked site](#manage-tracked-sites).
...
[![View cross-site activities in a contact’s activity log](/docsassets/documentation/cross-site-tracking/Cross-site_activities.png)](/docsassets/documentation/cross-site-tracking/Cross-site_activities.png)
Continue by extending the scripts according to your tracking requirements. Provide [consent functionality](#implement-tracking-consent) and implement [logging of additional activities](#log-cross-site-activities) to utilize all the cross site tracking capabilities.
...
To comply with [personal data protection](/documentation/developers-and-admins/data-protection) policies that apply to most websites, you need to prepare a consent and allow visitors to give consent agreements.
  ...
  2. Verify that the prepared tracking consent is assigned via `CrossSiteTrackingConsentOptions` when [enabling the cross-site tracking feature](#enable-the-cross-site-tracking-feature) for your Xperience application.
...
After preparing a tracking consent for your sites, extend the scripts in the [initial tracking snippet](#add-the-tracking-snippet) and build an interface to allow visitors to manage their consent agreements.
...
The following steps outline the general process for setting up tracking consent on external sites:
  ...
  5. [Handle error states](#handle-error-states) to provide information and a better user experience to visitors in cases where a problem occurs with the consent functionality.
...
Cross-site tracking allows you to log the following types of [activities](/documentation/developers-and-admins/digital-marketing-setup/set-up-activities) on external sites:
  * **Page visit** (logging included in the [initial tracking snippet](#add-the-tracking-snippet))
JS
Copy
```
...
You can optionally add [error state handling](#handle-error-states) to your activity logging calls. This allows you to record data for debugging and auditing purposes, or otherwise adjust your web application when a problem occurs while logging cross-site activities.
### Data input
...
To update contact data through _Data input_ activities, the [consent tracking options](#consent-management-on-external-sites) for the current request must have the `allow_datainput` option set to `true`.
...
## Handle error states
Most of the `kxt` cross-site tracking functions used for [consent management](#consent-management-on-external-sites) and [activity logging](#log-cross-site-activities) provide an optional `onerror` callback.
