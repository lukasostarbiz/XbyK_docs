---
source: https://docs.kentico.com/documentation/developers-and-admins/customization/customize-system-providers
...
The info provider customization pattern differs depending on provider type. See [Providers based on I*InfoProvider interfaces](#providers-based-on-iinfoprovider-interfaces) for dedicated providers and [Providers based on IInfoProvider<TInfo>](#providers-based-on-iinfoprovidertinfo) for generic providers. For general overview of both types of providers, see [Database table API](/documentation/developers-and-admins/api/database-table-api).
...
While using [providers based on IInfoProvider<TInfo>](#providers-based-on-iinfoprovidertinfo) is strongly recommended for all custom providers, not all system providers currently support this customization pattern. Use the approach described in this section to customize such providers.
