# Read-only deployments
  * [ Copy page link ](documentation/developers-and-admins/deployment/read-only-deployments#) | [Get HelpService ID](documentation/developers-and-admins/deployment/read-only-deployments#)
Core MVC 5


[✖](documentation/developers-and-admins/deployment/read-only-deployments# "Close page link panel") [Copy to clipboard](documentation/developers-and-admins/deployment/read-only-deployments#)
...
For [SaaS deployments](documentation/developers-and-admins/saas/saas-overview), Kentico’s deployment infrastructure automatically manages infrastructure requirements (deployment slots, database replication, blob versioning). However, you must still:
  1. **Configure your application code to support read-only mode** - See [Required project changes](documentation/developers-and-admins/deployment/read-only-deployments#required-project-changes) for mandatory code changes
...
This requirement extends to the implementations of the following **live site features** :
  ...
  * [Digital marketing](documentation/business-users/digital-marketing) – activity tracking, contact management, cross-site tracking. 
    * Email [subscription and unsubscription](documentation/developers-and-admins/digital-marketing-setup/enable-email-subscriptions) operations return HTTP 503 or redirect to a configured fallback page. See [Email marketing fallback pages](documentation/developers-and-admins/deployment/read-only-deployments#email-marketing-fallback-pages).
...
[![Read-only deployment timeline](docsassets/documentation/read-only-deployments/deployment-timeline.drawio.svg)](https://docs.kentico.com/docsassets/documentation/read-only-deployments/deployment-timeline.drawio.svg)
The deployment process follows these general steps:
  ...
  2. **Enable read-only mode on the clone** – Deploy the same application build (binaries) to a separate deployment slot and configure it to run in read-only mode. Set the application’s connection string to point to the cloned database and [configure](documentation/developers-and-admins/deployment/read-only-deployments#configure-read-only-mode-for-applications) the `VersioningTimestamp` to match the timestamp when the database was cloned. This ensures blob operations retrieve versions that align with the frozen database state.
