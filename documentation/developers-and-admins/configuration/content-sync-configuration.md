# Content sync configuration
  * [ Copy page link ](documentation/developers-and-admins/configuration/content-sync-configuration#) | [Get HelpService ID](documentation/developers-and-admins/configuration/content-sync-configuration#)
Core MVC 5


[✖](documentation/developers-and-admins/configuration/content-sync-configuration# "Close page link panel") [Copy to clipboard](documentation/developers-and-admins/configuration/content-sync-configuration#)
...
This page describes how to set up and configure your applications to use content sync:
  1. [Learn about the supported environment topologies](documentation/developers-and-admins/configuration/content-sync-configuration#supported-topologies)
  2. Set up content sync connections between environments: 
     * [Local environments and private cloud deployments](documentation/developers-and-admins/configuration/content-sync-configuration#set-up-content-sync)
     * [SaaS deployment environments](documentation/developers-and-admins/configuration/content-sync-configuration#content-sync-for-saas-environments)
  3. [Configure Xperience features and data for content sync](documentation/developers-and-admins/configuration/content-sync-configuration#configure-xperience-for-content-sync)


To learn how to synchronize content, see [Content sync](documentation/business-users/content-sync).
## Supported topologies
Before you start [setting up content sync](documentation/developers-and-admins/configuration/content-sync-configuration#set-up-content-sync), it is important to understand which environment topologies are supported.
...
[![Example of a supported content sync topology](docsassets/documentation/content-sync-configuration/content_sync_config_topology.png)](https://docs.kentico.com/docsassets/documentation/content-sync-configuration/content_sync_config_topology.png)
...
[![Examples of unsupported content sync topologies](docsassets/documentation/content-sync-configuration/content_sync_topologies_unsupported.png)](https://docs.kentico.com/docsassets/documentation/content-sync-configuration/content_sync_topologies_unsupported.png)
## Set up content sync
### Prerequisites
...
[SaaS deployment environments](documentation/developers-and-admins/deployment/deploy-to-the-saas-environment#deploy-the-package-to-another-deployment-environment) have their content sync configuration done automatically based on [Content synchronization connections](documentation/developers-and-admins/configuration/content-sync-configuration#manage-synchronization-connections) defined in Xperience Portal. However, the automatic configuration may be overridden by environment-specific configuration of `ContentSynchronizationOptions` as described in [Source and target configuration](documentation/developers-and-admins/configuration/content-sync-configuration#source-and-target-configuration).
...
The limitations described in [Supported topologies](documentation/developers-and-admins/configuration/content-sync-configuration#supported-topologies) apply for content sync between SaaS environments. Only linear connection chains are supported, without multiple branching target connections, loops, or bi-directional connections.
The connected environments must also be running exactly the same version of Xperience by Kentico.
  ...
  3. Choose the **Source** and **Target environment**. 
     * When setting up a **LOCAL** development environment as the source, see [Add connections from local development environments](documentation/developers-and-admins/configuration/content-sync-configuration#add-connections-from-local-development-environments).
  4. Select **Create**.


[![Adding a content synchronization connection](docsassets/documentation/content-sync-configuration/SaaS_content_sync_connections.png)](https://docs.kentico.com/docsassets/documentation/content-sync-configuration/SaaS_content_sync_connections.png)
...
When you create a [content sync connection](documentation/developers-and-admins/configuration/content-sync-configuration#manage-synchronization-connections) with a **LOCAL** environment as the source, Xperience Portal displays **Target URL** and **Connection secret** values. Copy the values and use them in the [content sync configuration](documentation/developers-and-admins/configuration/content-sync-configuration#source-and-target-configuration) of the application in your local development environment:
...
[![View planned or performed synchronizations](docsassets/documentation/content-sync-configuration/content_sync_tasks.png)](https://docs.kentico.com/docsassets/documentation/content-sync-configuration/content_sync_tasks.png)
