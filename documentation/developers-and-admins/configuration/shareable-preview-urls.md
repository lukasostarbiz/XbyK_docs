# Shareable preview URLs
  * [ Copy page link ](documentation/developers-and-admins/configuration/shareable-preview-urls#) | [Get HelpService ID](documentation/developers-and-admins/configuration/shareable-preview-urls#)
Core MVC 5


[✖](documentation/developers-and-admins/configuration/shareable-preview-urls# "Close page link panel") [Copy to clipboard](documentation/developers-and-admins/configuration/shareable-preview-urls#)
...
Anybody with the link can access the shareable preview. If your project uses Google Analytics or similar tracking tools, we recommend [excluding](documentation/developers-and-admins/configuration/shareable-preview-urls#exclude-shareable-preview-from-analytics) the shareable URLs from tracking to avoid unintentional leaks of the URLs to search engines.
...
To invalidate all shareable preview URLs in the system, [disable](documentation/developers-and-admins/configuration/shareable-preview-urls#disable-shareable-preview) the feature in the **Settings** application.
### URL changes
...
By default, shareable preview URLs automatically expire after 7 days if not [invalidated earlier](documentation/developers-and-admins/configuration/shareable-preview-urls#expiration). To change this setting:
  1. In the **Settings** application, go to **Content** → **URLs and SEO**.
  2. Set the desired **Expiration time** under **Shareable preview URLs**. 
     * You can set ‘0’ to prevent the default automatic expiration, however the URLs can still expire due to [other reasons](documentation/developers-and-admins/configuration/shareable-preview-urls#expiration).
  3. **Save** the changes.
