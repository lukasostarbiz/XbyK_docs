# Upgrade - Frequently asked questions
  * Concept| [ Copy page link ](guides/architecture/upgrade-from-kx13/upgrade-faq#) | [Get HelpService ID](guides/architecture/upgrade-from-kx13/upgrade-faq#)
Core MVC 5


[✖](guides/architecture/upgrade-from-kx13/upgrade-faq# "Close page link panel") [Copy to clipboard](guides/architecture/upgrade-from-kx13/upgrade-faq#)
...
Our [Learn portal (docs.kentico.com)](guides/architecture/upgrade-from-kx13/docs.kentico.com) provides end‑to‑end guidance on _how_ to use the Kentico Migration Tool. Our materials focus on implementation scenarios, upgrade planning, environment preparation, iterative data migrations, and other specifics.
As a starting point, see [Upgrade from Kentico Xperience 13](guides/architecture/upgrade-from-kx13). For advanced technical topics, like remodeling, check out our [Upgrade deep dives](guides/development/upgrade-deep-dives) for developers.
### Can I run the Kentico Migration Tool against my project multiple times?
Yes, data migration is designed to be iterative. Many teams run the tool, review the outcome, adjust the configuration, and run it again. Our [overview guide](guides/architecture/upgrade-from-kx13/upgrade-from-kx13-overview) illustrates the high‑level flow. Keep the scope small and run in batches - see the [GitHub README file](https://github.com/Kentico/xperience-by-kentico-kentico-migration-tool/blob/master/Migration.Tool.CLI/README.md#migrate-command-parameters) for list of parameters to migrate only certain objects.
...
You can configure selective migration in the `Migration.Tool.CLI/appsettings.json` file before each migration tool run. See in our [step‑by‑step walkthrough](guides/architecture/upgrade-from-kx13/upgrade-walkthrough/migrate-data-and-binary-files), where the configuration fits into the process.
...
For planning trade‑offs and feature differences, see our [capability comparison guide](guides/architecture/upgrade-from-kx13/plan-your-strategy-for-migrating-features).
...
When you run the data migration iteratively, we highly recommend taking database backups between runs; see the [_Can I run the Kentico Migration Tool against my project multiple times?_ question](guides/architecture/upgrade-from-kx13/upgrade-faq#can-i-run-the-kentico-migration-tool-against-my-project-multiple-times).
...
We’ve prepared a [guide to help you estimate the migration complexity](guides/architecture/upgrade-from-kx13/plan-your-strategy-for-migrating-features) based on the features your solution uses.
If your project uses e-commerce, see [our commerce project timeline considerations](guides/architecture/upgrade-from-kx13/upgrade-faq#what-are-the-current-estimates-regarding-timeline-and-costs-for-transitioning-from-kx13-to-xbyk-content-digital-commerce).
...
**Mitigation strategies** :
  ...
  * Allocate time for your developers to learn the Kentico Migration Tool using our hands-on [upgrade walkthrough](guides/architecture/upgrade-from-kx13/upgrade-walkthrough), [technical deep-dive scenarios](guides/development/upgrade-deep-dives), and the [GitHub documentation](https://github.com/Kentico/xperience-by-kentico-kentico-migration-tool/blob/master/README.md).
...
For a detailed comparison of both systems, refer to our [Plan your strategy for migrating features](guides/architecture/upgrade-from-kx13/plan-your-strategy-for-migrating-features) guide.
...
It is technically possible to migrate commerce widgets from previous Kentico versions. However, the level of customization required during the upgrade process depends on both the Kentico version and the customizations implemented in the Kentico source project.
  * **Review the project**. Consult the [Upgrade overview](guides/architecture/upgrade-from-kx13/upgrade-from-kx13-overview) for best practices, supported scenarios, and detailed steps to ensure a smooth data transition.  

  * **Data source changes**. To make widgets functional, you will need to [adapt their data sources and retrieval](guides/architecture/upgrade-from-kx13/adjust-your-code-and-adapt) to align with XbyK’s API and data model.
  * **KX13 widgets**. Migrating commerce widgets from KX13 follows the same process as other widget migrations. You can transfer the data using Kentico Migration Tool, and then update your code that [retrieves the data](guides/development/upgrade-deep-dives/upgrade-content-retrieval). For general recommendations about migrating widgets, see [Upgrade widgets](guides/development/upgrade-deep-dives/upgrade-widgets-introduction) guide.
...
Recommended approach: Instead of migrating KX13 widgets “as-is”, consider [remodeling and rebuilding the widgets](guides/development/upgrade-deep-dives#work-with-widgets-during-upgrade) during migration to ensure a consistent look and feel, better performance, and long-term maintainability.
...
However, please note:
  * We **do not have a complex commerce demo site** (similar to the KBank demo in KX13) that showcases a full end-to-end commerce implementation.
...
### Will the migration tool support order and customer migration (mapping)?
Yes, we plan to include order and customer migration in [Kentico Migration Tool](documentation/developers-and-admins/upgrade-to-xperience-by-kentico/migration-toolkit). We expect the support to be available by the end of January 2026.
This enhancement will help streamline the migration process from KX 13 to XbyK for projects that rely on customer and order data. For updates, please keep an eye on the [product roadmap](https://roadmap.kentico.com/tabs/6-next-refreshes).
