# Automate regular tasks with PowerShell scripts
  * [ Copy page link ](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#) | [Get HelpService ID](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#)
Core MVC 5


[✖](modules/developer-setup/automate-regular-tasks-with-powershell-scripts# "Close page link panel") [Copy to clipboard](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#)
...
Visit our [Code generator documentation](documentation/developers-and-admins/api/generate-code-files-for-system-objects) to learn more about its parameters and usage. Additionally, you can see a more detailed exploration and examples [in this video](modules/developer-setup/generate-code-for-custom-content-and-data-classes). 
## Publish
...
This guide’s example closely follows the PowerShell script provided by the documentation. Start by copying the script and changing it in three key ways.
  ...
  * Use the `Get-ConnectionString` function from the [previous section](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#connection-string-function), rather than including the function innately.
...
Information about the migrations is logged into the `CI_Migration` table of the database, which you can check to ensure that the same migration does not run multiple times.
  ...
  * `Run-Restore`
    ...
    * Runs a CI restore.
Just like the [CI store script above](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#ci-store), CI restore also uses the optional `--no-build` parameter. Consider your team’s procedures when deciding whether to include it. 
...
If the _CI store_ operation fails when you run this script, we highly recommend taking the following steps after addressing the issue that led to failure:
  ...
  2. Running [CI store](modules/developer-setup/automate-regular-tasks-with-powershell-scripts#ci-store)
...
[ Previous page ](modules/developer-setup/set-up-continuous-integration-and-developer-workflow)
6 of 7
[ Mark complete and continue ](modules/developer-setup/developer-setup-learn-more)
...
USE ONLY NECESSARY 
![](https://docs.kentico.com/modules/developer-setup/automate-regular-tasks-with-powershell-scripts)
[](https://docs.kentico.com/modules/developer-setup/automate-regular-tasks-with-powershell-scripts)[](https://docs.kentico.com/modules/developer-setup/automate-regular-tasks-with-powershell-scripts)